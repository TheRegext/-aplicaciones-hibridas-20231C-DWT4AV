...
function createProductListPage(products) {
    ...
    for (let i = 0; i < products.length; i++) {
        html += `<li>${products[i].name} <a href="/products/${products[i].id}">Ver</a> <a href="/products/${products[i].id}/edit">Modifcar</a> <a href="/products/${products[i].id}/delete">Eliminar</a> </li>`
    }
...
function formEditProduct(product) {
    ...
    html += `<form action="/products/${product.id}/edit" method="POST" enctype="application/x-www-form-urlencoded">
        <label for="name">Nombre: 
...
function formDeleteProduct(product) {
    let html = `<h1>Eliminar producto #${product.id}</h1>`

    html += `<h1>${product.name}</h1>`
    html += `<p>Precio: ${product.price}</p>`
    html += `<p>${product.description}</p>`

    html += `<form action="/products/${product.id}/delete" method="POST" enctype="application/x-www-form-urlencoded">
